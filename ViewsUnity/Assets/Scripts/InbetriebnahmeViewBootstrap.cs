...
public sealed class InbetriebnahmeViewBootstrap : MonoBehaviour
{
    ...
    private void BuildView()
    {
        ...
        closeButton.onClick.AddListener(EnterRoom);
    ...
    private async Task<Dictionary<string, string>> ReadControllerInfoFromOpcUa()
    {
        ...
        {
            ApplicationName = "Inbetriebnahme View",
            ApplicationUri = "urn:localhost:InbetriebnahmeView",
            ...
            SecurityConfiguration = new SecurityConfiguration
            {
                ...
                {
                    StoreType = "Directory",
                    StorePath = Path.Combine(Application.persistentDataPath, "opcua", "own"),
                    SubjectName = "CN=Inbetriebnahme View"
                },
                TrustedPeerCertificates = new CertificateTrustList
                {
                    StoreType = "Directory",
                    StorePath = Path.Combine(Application.persistentDataPath, "opcua", "trusted")
                },
                RejectedCertificateStore = new CertificateTrustList
                {
                    StoreType = "Directory",
                    StorePath = Path.Combine(Application.persistentDataPath, "opcua", "rejected")
                }
            },
        ...
        config.CertificateValidator.CertificateValidation += (_, eventArgs) => eventArgs.Accept = true;

        var endpointDescription = CoreClientUtils.SelectEndpoint(config, OpcUaEndpointUrl, false, 5000);
...
{
    ["CurrentSession..."] = new NodeId(2277, 0),  // Server_ServerDiagnostics_CurrentSessionCount
    ["CurrentTime"]       = new NodeId(2258, 0),  // Server_ServerStatus_CurrentTime
    ["State"]             = new NodeId(2259, 0),  // Server_ServerStatus_State
    ["SoftwareVersion"]   = new NodeId(2264, 0),  // Server_ServerStatus_BuildInfo_SoftwareVersion
    ["ManufacturerN..."]  = new NodeId(2263, 0),  // Server_ServerStatus_BuildInfo_ManufacturerName
    ["BuildDate"]         = new NodeId(2265, 0),  // Server_ServerStatus_BuildInfo_BuildDate
    ["ServerArray"]       = new NodeId(2254, 0)   // Server_ServerArray
};

        var cpuModelNode = FindNodeByBrowsePath(session, ObjectIds.ObjectsFolder, new[] { "Resources", "CPU", "Model" });
        if (!NodeId.IsNull(cpuModelNode))
        {
            nodeIds["Model"] = cpuModelNode;
        }

        var serialNumberNode = FindNodeByBrowsePath(session, ObjectIds.ObjectsFolder, new[] { "Resources", "CPU", "SerialNumber" });
        if (!NodeId.IsNull(serialNumberNode))
        {
            nodeIds["SerialNumber"] = serialNumberNode;
        }
        ...
        {
            ["Model"] = NodeId.IsNull(cpuModelNode) ? "NodeId fehlt - bitte exakten UAExpert NodeId angeben" : "Nicht gelesen",
            ["SerialNumber"] = NodeId.IsNull(serialNumberNode) ? "NodeId fehlt - bitte exakten UAExpert NodeId angeben" : "Nicht gelesen"
        };

        for (var i = 0; i < results.Count; i++)
        {
            values[order[i]] = StatusCode.IsGood(results[i].StatusCode)
                ? FormatOpcValue(results[i].Value)
    ...
    private static NodeId FindNodeByBrowsePath(Session session, NodeId startNode, string[] browseNames)
    {
        var current = startNode;
        foreach (var browseName in browseNames)
        {
            current = FindChildByBrowseName(session, current, browseName);
            if (NodeId.IsNull(current))
            {
                return NodeId.Null;
            }
        }
    ...
    private static NodeId FindChildByBrowseName(Session session, NodeId parent, string browseName)
    {
        session.Browse(
            null,
            null,
        ...
        {
            if (string.Equals(reference.BrowseName.Name, browseName, StringComparison.OrdinalIgnoreCase))
            {
